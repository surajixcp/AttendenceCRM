    ...
    <div className="space-y-4 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center space-x-1.5 md:space-x-2 bg-white dark:bg-slate-900/40 p-1 md:p-1.5 rounded-lg shadow-sm border border-slate-100 dark:border-slate-800/50 backdrop-blur-xl shrink-0">
            <div className="flex items-center px-0.5 md:px-1">
              <input
                type="date"
                className="bg-transparent text-[9px] md:text-[10px] font-black text-slate-600 dark:text-slate-300 outline-none cursor-pointer uppercase tracking-tight w-20 md:w-auto"
                value={startDate}
            ...
            <div className="flex items-center px-0.5 md:px-1">
              <select
                className="bg-transparent text-[9px] md:text-[10px] font-black text-slate-600 dark:text-slate-300 outline-none cursor-pointer uppercase tracking-tight max-w-[80px] md:max-w-[120px]"
                value={selectedUser}
                onChange={(e) => setSelectedUser(e.target.value)}
              >
                <option value="" className="bg-white dark:bg-slate-900">ALL</option>
                {employees.map((emp) => (
            ...
            <button
              onClick={fetchLogs}
              className="p-1 md:p-1.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all active:scale-90"
          ...
          <button
            ...
            className="flex items-center justify-center bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800/50 hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 px-3 md:px-4 py-2 md:py-2.5 rounded-lg font-black text-[8px] md:text-[9px] uppercase tracking-widest shadow-sm transition-all active:scale-95 group shrink-0 disabled:opacity-50"
          >
            {isExporting ? (
              <div className="w-2.5 md:w-3 h-2.5 md:h-3 border-2 border-blue-500/20 border-t-blue-500 rounded-full animate-spin mr-1 md:mr-1.5"></div>
            ) : (
              <Icons.Download className="w-2.5 md:w-3 h-2.5 md:h-3 mr-1 md:mr-1.5 group-hover:-translate-y-0.5 transition-transform" />
            )}
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50 dark:bg-slate-950/50 border-b border-slate-100 dark:border-slate-800/50">
              <tr className="text-slate-400 dark:text-slate-500 text-[8px] md:text-[9px] font-black uppercase tracking-widest">
                <th className="px-3 md:px-5 py-2.5 md:py-3.5">Staff</th>
                <th className="px-5 py-3.5 hidden md:table-cell">Temporal Marker</th>
                <th className="px-3 md:px-5 py-2.5 md:py-3.5 text-center">In</th>
                <th className="px-3 md:px-5 py-2.5 md:py-3.5 text-center">Out</th>
                <th className="px-5 py-3.5 text-center hidden md:table-cell">Utilization</th>
                <th className="px-3 md:px-5 py-2.5 md:py-3.5 text-right">Verify</th>
              </tr>
            </thead>
                ...
                <tr key={record._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all group">
                  <td className="px-3 md:px-5 py-2.5 md:py-3 whitespace-nowrap">
                    <div className="flex flex-col leading-tight">
                      <span className="text-[10px] md:text-[11px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                        {record.user?.name || 'Incomplete Identity'}
                      </span>
                      <span className="text-[7px] md:text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5 truncate max-w-[100px] md:max-w-none">
                  ...
                  <td className="px-3 md:px-5 py-2.5 md:py-3 whitespace-nowrap text-center">
                    <span className="text-[9px] md:text-[10px] font-black text-slate-700 dark:text-slate-300 font-mono">
                  ...
                  <td className="px-3 md:px-5 py-2.5 md:py-3 whitespace-nowrap text-right">
                    <div className="flex flex-col items-end gap-0.5 md:gap-1">
                      <span className={`inline-flex px-1 md:px-1.5 py-0.5 text-[7px] md:text-[8px] font-black uppercase tracking-widest rounded-md ${record.status === 'present' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-500/20' :
